...
# Write your code here
str_c(str_sub(pig_latin, start = 2), str_sub(pig_latin, star = 1, end =1), "ay", collapse = " ")
```
...
# Write your code here
secret_message <- str_sub(secret_message, end =  -3)
str_c(str_sub(secret_message, start =  -1), str_sub(secret_message, start = 1, end = -2), collapse = " ")
```
...
3. Create a new column in `news_df2` called `text_len` that contains the length of the character variable `text`.
    ...
    - What is the `class` and `type` of this new column? Make sure to include your code in the code chunk below.
      - __ANSWER__: The class of `text_len` is "integer", the type of `text_len` is "integer".
    
```{r}
news_df2 <- news_df2 %>%
  mutate(text_len = str_length(text))  #another way: text_length = nchar(news_df2$text)
...
4. Create an additional column in `news_df2` called `handle_followers` that stores the twitter handle and the number of followers associated with that twitter handle in a string. For example, the entries in the `handle_followers` column should look like this: `@[twitter_handle] has [number] followers.`
    ...
    - What is the `class` and `type` of this new column? Make sure to include your code in the code chunk below.
      - __ANSWER__: The class is "character", the type is "character."
...
```{r}
news_df2 <- news_df2 %>%
  mutate(short_web = str_remove(profile_expanded_url, "http://www."))

head(news_df2$short_web)
```
...
news_df2 <- news_df2 %>%
  mutate(
    ...
    time_chr = str_sub(dt_chr, start = 12)
  )
```
...
news_df2 <- news_df2 %>%
  mutate(
    yr_num = year(ymd(date_chr)),
    mth_num = month(ymd(date_chr)),
    day_num = day(ymd(date_chr)),
    hr_num = hour(hms(time_chr)),
    min_num = minute(hms(time_chr)),
    sec_num = second(hms(time_chr))
  )
```
...
6. Using the **new numeric columns** (e.g. day_num, mth_num) you've created in the previous step, reconstruct the date and datetime columns. Namely, add the following columns to `news_df2`:
    ...
    - What is the `class` of your `my_date` and `my_datetime` columns? Make sure to include your code in the code chunk below.
      - __ANSWER__: The class of `my_date` is "Date", the class of  `my_datetime` is "POSIX" (POSIXct and POSIXt).

```{r}
news_df2 <- news_df2 %>%
  mutate(
    my_date = make_date(yr_num, mth_num, day_num),
    my_datetime = make_datetime(yr_num, mth_num, day_num, hr_num, min_num, sec_num)
  )

class(news_df2$my_date)
class(news_df2$my_datetime)
```
...
- Paste the url to your issue here: https://github.com/anyone-can-cook/rclass1_student_issues_f23/issues/645
- Paste the url to the issue you responded to here: https://github.com/anyone-can-cook/rclass1_student_issues_f23/issues/640
