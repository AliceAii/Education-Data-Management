...
You'll also be practicing variable creation in this problem set, using both `mutate()` in combination with `if_else()`, `case_when()`, and `recode()`, as well as the base R approach.
...
```{r}
df_school_all2 <- select(arrange(df_school_all, desc(total_visits)), name, state_code, city, 
                         school_type, total_visits, med_inc, pct_white, pct_black, 
                         pct_hispanic, pct_asian, pct_amerindian)
head(df_school_all2, 10)
```

__Using pipes ('%>%')__:

```{r}
df_school_all %>% arrange(desc(total_visits)) %>%
...
```{r}
head(filter(df_school_all2, school_type == "public" & state_code == "CA"), 10)
head(filter(df_school_all2, school_type == "private" & state_code == "CA"), 10)
```

__Using pipes ('%>%')__:

```{r}
df_school_all2 %>% filter(school_type == "public" & state_code == "CA") %>% head(10)
df_school_all2 %>% filter(school_type == "private" & state_code == "CA") %>% head(10)
```
...
```{r}
df_school_all %>% mutate(ca_school = if_else(state_code == "CA", 1, 0)) %>% 
  count(ca_school)
...
```{r}
df_race <- df_school_all %>% select(name, pct_black, pct_hispanic, pct_amerindian)

df_race %>% filter(is.na(pct_black) | is.na(pct_hispanic) | is.na(pct_amerindian)) %>%
  count() 

df_race <- df_race %>% 
  mutate(pct_bl_hisp_nat = pct_black + pct_hispanic + pct_amerindian)

df_race %>% mutate(gt50pct_bl_hisp_nat = if_else(pct_bl_hisp_nat >50, 1, 0)) %>%
...
# Question 3: Variable creation using base R approach

1. Use the `df_race` dataframe from Question 2.2 to complete the following:

    - Create a new variable called `pct_bl_hisp_nat_baseR` in `df_race` that is the sum of `pct_black`, `pct_hispanic`, and `pct_amerindian`. You should get the same values as the `pct_bl_hisp_nat` variable you created in Question 2.2.
    - Create a new variable called `bl_hisp_nat_cat` in `df_race` whose value is:
      - `high`, if `pct_bl_hisp_nat_baseR` is greater than or equal to `50`
      - `medium`, if `pct_bl_hisp_nat_baseR` is less than `50` and greater than or equal to `15`
      - `low`, if `pct_bl_hisp_nat_baseR` is less than `15`

```{r}
df_race <- df_race %>%
  mutate(pct_bl_hisp_nat_baseR = pct_black + pct_hispanic + pct_amerindian)

df_race <- df_race %>% 
  mutate(bl_hisp_nat_cat = case_when(
    pct_bl_hisp_nat_baseR >= 50 ~ 'high',
    pct_bl_hisp_nat_baseR < 50 & pct_bl_hisp_nat_baseR >= 15 ~ 'medium',
    pct_bl_hisp_nat_baseR < 15 ~ 'low'
  ))
...
- Paste the url to your issue here: https://github.com/anyone-can-cook/rclass1_student_issues_f23/issues/433
- Paste the url to the issue you responded to here: https://github.com/anyone-can-cook/rclass1_student_issues_f23/issues/431
