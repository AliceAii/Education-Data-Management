...
```{r}
nrow(filter(df_event, univ_id == 166629 & event_type == "public hs" &
                    event_state != instst & med_inc >= 100000))
```
...
```{r}
df_events <- arrange(df_event, univ_id, event_date, event_state, desc(pct_white_zip), desc(med_inc))
head(df_events, 10)
```
...
```{r}
df_events1 <- select(df_event, univ_id, event_date, event_type, zip, med_inc)
names(df_events1)
```

__base R using subsetting operators__:
```{r}
df_events2 <- df_event[ , c("univ_id", "event_date", "event_type", "zip", "med_inc")]
names(df_events2)
```
...
```{r}
df_school_all2 <- select(arrange(df_school_all, desc(total_visits)), name, state_code, city, school_type, total_visits, 
...
- Paste the url to your issue here: https://github.com/anyone-can-cook/rclass1_student_issues_f23/issues/332
- Paste the url to the issue you responded to here: https://github.com/anyone-can-cook/rclass1_student_issues_f23/issues/321
